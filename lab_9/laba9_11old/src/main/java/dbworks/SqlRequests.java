...
public class SqlRequests {
	...
	public String showSecondFieldInAllBooks() {
		try {
			Statement statement = connection.createStatement();
			ResultSet resultSet = statement.executeQuery( "select publication from book");
			StringBuilder table = new StringBuilder("<html>publication:<br>");
			while (resultSet.next()) {
	...
	public String showPublishingHouseInWardrobe(int floor, int wardrobe) {
		try {
			Statement statement = connection.createStatement();
			ResultSet resultSet = statement.executeQuery(
					"select publishing_house from book where place_id in(select id from place where" +
							" floor = " + floor + " and wardrobe = " + wardrobe + ") order by publishing_house");
			StringBuilder table = new StringBuilder("<html>publishing_house:<br>");
			while (resultSet.next()) {
				table.append(resultSet.getString("publishing_house"))
						.append("<br>");
