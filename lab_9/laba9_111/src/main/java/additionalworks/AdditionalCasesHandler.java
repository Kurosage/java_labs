...
public class AdditionalCasesHandler {
	...
	private void initializeButtons() {
		...
		thirdFieldByLexicOrderButton = new JButton("Получить 3-е поле аудитории в лексикографическом порядке");
		additionalPanel.add(thirdFieldByLexicOrderButton);
		secondFieldInClassRoomButton = new JButton("Получить 2 поле аудитории");
		additionalPanel.add(secondFieldInClassRoomButton);
		showNumbersButton = new JButton("Получить список аудиторий в указанном здании");
		additionalPanel.add(showNumbersButton);
		totalSurfaceButton = new JButton("Получить общую площадь аудиторий закреплённых за указанным ответственным");
	...
	private void getTotalSurfaceToPersonAction() {
		totalSurfaceButton.addActionListener(e -> {
			...
			labelShowTable.setText("Общее количество квадратных метров: " + Double.toString(sqlRequests.showTotalSurfaceInPerson(
